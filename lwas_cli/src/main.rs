...
async fn handle_reality_map(State(state): State<Arc<AppState>>) -> Json<RealityMapResponse> {
    ...
    let project_path =
        std::path::PathBuf::from(std::env::var("PROJECT_PATH").unwrap_or_else(|_| ".".to_string()));
    let dossier_path = std::path::PathBuf::from(
        std::env::var("DOSSIER_PATH").unwrap_or_else(|_| "/tmp/aeterna_dossier".to_string()),
    );
