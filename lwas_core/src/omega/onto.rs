...
impl SovereignOntoEngine {
    ...
    /// COMPLEXITY: O(1) - DashMap insert is amortized constant time. Vector projection is O(L) where L is expression length (negligible).
    ...
    /// COMPLEXITY: O(N) - Linear scan of expression bytes.
